...
export class PluginDriver {
  ...
  constructor(
    ...
    this.pluginContexts = new Map(
      this.plugins.map(plugin => [
        plugin,
        getPluginContext(plugin, graph, options)
      ])
    );
  }
  ...
  async executeFromPluginHook<H extends keyof FunctionPluginHooks>(
    ...
    if (!fromPlugin.length || fromPlugin.length > 1) {
      out.error(`Error running plugin hook "${hookName}", expected exactly one plugin to handle this hook.`)
    }
...
function validateFunctionPluginHandler(handler: unknown, hookName: string, plugin: IPlugin) {
  if (typeof handler !== 'function') {
    out.error(`Error running plugin hook "${hookName}" for plugin "${plugin}", expected a string, a function hook or an object with a "handler" string or function.`)
  }
}
